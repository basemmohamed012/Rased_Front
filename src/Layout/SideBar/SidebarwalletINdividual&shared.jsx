...
const SidebarWallet = () => {
  ...
  return (
    <div className="w-[259px] h-[2000px] relative top-[10px] right-[40px] bg-gradient-to-b from-[#415C36] to-[#506F66] text-white rounded-2xl p-4 flex flex-col gap-4">
      ...
      {/* المحافظ */}
      <div
        className={`relative right-1 mt-4 transition-all duration-300 ${dropdownOpen ? 'mb-16' : ''}`}
        ref={dropdownRef}
      >
        <button
          onClick={() => {
          ...
          className={`w-[196px] h-[36px] rounded-[15px] flex items-center justify-between px-3 py-2 transition-all duration-300 ${
            location.pathname === '/showWalletIndividual' || '/ShowSharedWallet' ? 'bg-white text-black' : 'text-white'
          }`}
        >
          <div className="flex gap-2 items-center">
            <img src={ wallet } alt="محافظي" />
        ...
        {dropdownOpen && (
          <div className="absolute top-full mt-2 w-[196px] bg-white rounded-[10px] shadow-lg py-2 z-10">
           <button
              onClick={() => {
                Wallets(); 
                dropdownOpen(true);
